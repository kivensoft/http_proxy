...
use hyper::{
    ...
    StatusCode, Uri, body::to_bytes, Request, Body,
};
...
type ServiceInfoList = VecDeque<CompactString>;
...
pub fn init_client(val: Option<Duration>) {
    ...
    CLIENT.set(Client::builder().build(hc)).expect("初始化http客户端错误");
}
...
pub fn register_service(path: &str, endpoint: &str) -> bool {
    ...
    match services.get_mut(path) {
        // 路径有注册的服务
        Some(svr_list) => match svr_list.iter_mut().find(|v| v.as_str() == endpoint) {
            // 对应端点的服务找到，更新过期时间
            Some(_) => ret = false,
            // 找不到，创建服务并添加到链表末尾
            None => svr_list.push_back(endpoint.to_compact_string()),
        },
        // 路径对应没有服务，创建服务及链表
        None => {
            let mut slist = ServiceInfoList::new();
            slist.push_back(endpoint.to_compact_string());
...
pub fn unregister_service(path: &str, endpoint: &str) {
    ...
    if let Some(svr_list) = svr_list {
        svr_list.retain(|v| v.as_str() != endpoint);
...
pub fn service_status() -> Vec<ServiceGroup> {
    SERVICES.lock().iter()
        .map(|(k, v)| {
            let services = v.iter().map(|v| v.clone()).collect();
            ServiceGroup {
...
pub async fn proxy_handler(ctx: HttpContext) -> Result<Response> {
    let mut ctx = log_request(ctx).await;
    ...
    let path_and_query = ctx.req.uri().path_and_query().map(|v| v.as_str()).unwrap_or("/");
    ...
    *ctx.req.uri_mut() = uri;
    ...
    match client.request(ctx.req).await {
        Ok(r) => Ok(log_response(req_id, r).await),
        Err(e) => {
...
fn get_service_endpoint(path: &str) -> Option<CompactString> {
    ...
    while let Some(pos2) = pos {
        ...
        if let Some(svr_list) = services.get_mut(path) {
            // 找到第一个未过期的接口服务
            if let Some(svr) = svr_list.pop_front() {
                // 服务未过期，返回其端点地址并将其重新放到服务链表尾部
                svr_list.push_back(svr.clone());
                return Some(svr);
            }
        }
...
fn unregister_service_by_endpoint(endpoint: &str) {
    SERVICES.lock().retain(|_, v| {
        v.retain(|s| s.as_str() != endpoint);
        !v.is_empty()
    });
}

async fn log_request(mut ctx: HttpContext) -> HttpContext {
    if !log::log_enabled!(log::Level::Trace) { return ctx; }
    ...
    ctx.req = Request::from_parts(parts, body);
    ctx
}

async fn log_response(req_id: u32, res: Response) -> Response {
    if !log::log_enabled!(log::Level::Trace) { return res; }
